...
(defun* feature-run-cucumber (cuke-opts &key feature-file)
  ...
  (let ((opts-str    (mapconcat 'identity cuke-opts " "))
    ...
    (let ((default-directory (feature-project-root))
      ...
      (compile (concat (replace-regexp-in-string "CUCUMBER_OPTS=\"\"" "" (replace-regexp-in-string "{options}" 
                                                 opts-str
                                                 (replace-regexp-in-string "{feature}" feature-arg feature-cucumber-command) 
                                                 t 
                                                 t))) t))))
